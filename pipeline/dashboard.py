...
<head>
    ...
    <style>
        :root {
            ...
            --row-dropped-bg: #1c0a0a; --row-dropped-text: #fca5a5;
            --row-modified-bg: #0c1a2e;
            --legend-dropped-bg: #450a0a; --legend-modified-bg: #0c1a2e;
            --badge-blue-bg: #1e3a5f; --badge-green-bg: #14532d;
            --badge-red-bg: #450a0a; --badge-yellow-bg: #422006; --badge-orange-bg: #431407;
            --icon-drop-bg: #450a0a; --icon-fix-bg: #14532d;
            --icon-dedup-bg: #422006; --icon-add-bg: #1e3a5f; --icon-warn-bg: #431407;
        }
        [data-theme="light"] {
            ...
            --row-dropped-bg: #fef2f2; --row-dropped-text: #991b1b;
            --row-modified-bg: #eff6ff;
            --legend-dropped-bg: #fecaca; --legend-modified-bg: #bfdbfe;
            --badge-blue-bg: #dbeafe; --badge-green-bg: #dcfce7;
            --badge-red-bg: #fee2e2; --badge-yellow-bg: #fef9c3; --badge-orange-bg: #ffedd5;
            --icon-drop-bg: #fee2e2; --icon-fix-bg: #dcfce7;
            --icon-dedup-bg: #fef9c3; --icon-add-bg: #dbeafe; --icon-warn-bg: #ffedd5;
        }
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .page-tabs { display: flex; gap: 0; background: var(--surface); border-bottom: 1px solid var(--border); padding: 0 2rem; }
        .page-tab { padding: 0.85rem 1.5rem; cursor: pointer; border-bottom: 3px solid transparent; color: var(--text-muted); font-size: 0.95rem; font-weight: 600; transition: all 0.15s; background: none; border-top: none; border-left: none; border-right: none; }
        .page-tab:hover { color: var(--text); }
        .page-tab.active { color: var(--accent-light); border-bottom-color: var(--accent-light); }
        ...
        .stat-card .detail { font-size: 0.8rem; color: var(--text-muted); margin-top: 0.25rem; }
        ...
        .changelog { display: grid; gap: 0.75rem; }
        .change-item { background: var(--surface); border: 1px solid var(--border); border-radius: 0.75rem; padding: 1rem 1.25rem; display: flex; align-items: flex-start; gap: 1rem; }
        .change-icon { width: 2rem; height: 2rem; border-radius: 0.5rem; display: flex; align-items: center; justify-content: center; font-size: 1rem; flex-shrink: 0; margin-top: 0.1rem; }
        ...
        .change-body h4 { font-size: 0.95rem; font-weight: 600; margin-bottom: 0.2rem; }
        .change-body p { font-size: 0.85rem; color: var(--text-muted); }
        .change-body .affected { font-size: 0.8rem; color: var(--text-muted); margin-top: 0.4rem; font-style: italic; }
        .tabs { display: flex; gap: 0; border-bottom: 1px solid var(--border); margin-bottom: 1rem; }
        .tab { padding: 0.75rem 1.25rem; cursor: pointer; border-bottom: 2px solid transparent; color: var(--text-muted); font-size: 0.9rem; font-weight: 500; transition: all 0.15s; background: none; border-top: none; border-left: none; border-right: none; }
        .tab:hover { color: var(--text); }
        .tab.active { color: var(--accent-light); border-bottom-color: var(--accent-light); }
        .tab-content { display: none; }
        .tab-content.active { display: block; }
        .table-wrapper { overflow-x: auto; border: 1px solid var(--border); border-radius: 0.75rem; }
        table { width: 100%; border-collapse: collapse; font-size: 0.85rem; }
        thead { background: var(--surface); position: sticky; top: 0; }
        th { text-align: left; padding: 0.75rem 1rem; font-weight: 600; color: var(--text-muted); font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.05em; border-bottom: 1px solid var(--border); }
        td { padding: 0.6rem 1rem; border-bottom: 1px solid var(--border); white-space: nowrap; max-width: 300px; overflow: hidden; text-overflow: ellipsis; }
        tr:last-child td { border-bottom: none; }
        tr:hover td { background: var(--surface-hover); }
        tr.row-dropped td { background: var(--row-dropped-bg); color: var(--row-dropped-text); text-decoration: line-through; opacity: 0.7; }
        tr.row-modified td { background: var(--row-modified-bg); }
        tr.row-modified td.cell-changed { color: var(--accent-light); font-weight: 600; }
        ...
        .legend-dot.modified { background: var(--legend-modified-bg); border: 1px solid var(--accent); }
        .legend-dot.unchanged { background: var(--surface); border: 1px solid var(--border); }
        .category-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 0.75rem; }
        .cat-card { background: var(--surface); border: 1px solid var(--border); border-radius: 0.75rem; padding: 1rem; }
        .cat-card .cat-name { font-size: 0.85rem; font-weight: 600; text-transform: capitalize; margin-bottom: 0.5rem; }
        .cat-card .cat-stat { font-size: 0.8rem; color: var(--text-muted); }
        .cat-card .cat-stat span { color: var(--text); font-weight: 600; }
        .theme-toggle { background: var(--surface-hover); border: 1px solid var(--border); color: var(--text); padding: 0.5rem 1rem; border-radius: 0.5rem; cursor: pointer; font-size: 0.85rem; font-weight: 500; display: flex; align-items: center; gap: 0.4rem; transition: all 0.2s; }
        .theme-toggle:hover { background: var(--border); }
        .charts-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 2rem; }
        @media (max-width: 900px) { .charts-grid { grid-template-columns: 1fr; } }
        .chart-card { background: var(--surface); border: 1px solid var(--border); border-radius: 0.75rem; padding: 1.5rem; }
...
<body>
    ...
    <div class="page-tabs">
        <button class="page-tab active" onclick="switchPage('overview', this)">Overview</button>
        <button class="page-tab" onclick="switchPage('data', this)">Data Comparison</button>
        <button class="page-tab" onclick="switchPage('charts', this)">Charts</button>
    </div>
    ...
    <div id="page-overview" class="page-content active">
        <div class="container">
            ...
            <div class="section">
                <div class="section-header"><h2>Transformation Changelog</h2></div>
                <div class="changelog">
            ...
            <div class="section">
                <div class="section-header"><h2>Category Breakdown (After Processing)</h2></div>
    ...
    <div id="page-data" class="page-content">
        <div class="container">
            <div class="section">
                <div class="tabs">
                    <button class="tab active" onclick="switchTab('before', this)">
                        Raw Input <span class="badge badge-blue">{{ stats.input_rows }} rows</span>
                    </button>
                    <button class="tab" onclick="switchTab('after', this)">
                        Processed Output <span class="badge badge-green">{{ stats.output_rows }}</span>
                    </button>
                    <button class="tab" onclick="switchTab('diff', this)">
                ...
                <div id="tab-before" class="tab-content active">
                    <div class="table-wrapper">
                        <table>
                            <thead><tr>
                                {% for col in before_columns %}<th>{{ col }}</th>{% endfor %}
                            </tr></thead>
                            <tbody>
                                {% for row in before_data %}
                                <tr>{% for col in before_columns %}<td>{{ row[col] }}</td>{% endfor %}</tr>
                                {% endfor %}
                            </tbody>
                        </table>
                    </div>
                ...
                <div id="tab-after" class="tab-content">
                    <div class="table-wrapper">
                        <table>
                            <thead><tr>
                                {% for col in after_columns %}<th>{{ col }}</th>{% endfor %}
                            </tr></thead>
                            <tbody>
                                {% for row in after_data %}
                                <tr>{% for col in after_columns %}<td>{{ row[col] }}</td>{% endfor %}</tr>
                                {% endfor %}
                            </tbody>
                        </table>
                    </div>
                ...
                <div id="tab-diff" class="tab-content">
                    <div class="legend">
                        <div class="legend-item"><div class="legend-dot dropped"></div> Dropped</div>
                        <div class="legend-item"><div class="legend-dot modified"></div> Modified</div>
                        <div class="legend-item"><div class="legend-dot unchanged"></div> Unchanged</div>
                    </div>
                    <div class="table-wrapper">
                        <table>
                            <thead><tr>
                                <th>Status</th><th>ID</th>
                                {% for col in diff_columns %}<th>{{ col }}</th>{% endfor %}
                                <th>Changes</th>
                            </tr></thead>
                            <tbody>
                                ...
                                <tr class="{{ row.row_class }}">
                                    ...
                                    <td class="{{ 'cell-changed' if col in row.changed_cols else '' }}">{{ row.values[col] }}</td>
                                    {% endfor %}
    ...
    <div id="page-charts" class="page-content">
        <div class="container">
            <div class="charts-grid">
                ...
                <div class="chart-card">
                    <h3>Row Status Distribution</h3>
                    <canvas id="chart-status-pie"></canvas>
                </div>
                <div class="chart-card">
                    <h3>Row Count by Category</h3>
                    <canvas id="chart-category-count"></canvas>
                </div>
                <div class="chart-card">
                    <h3>Transformation Impact</h3>
                    <canvas id="chart-transform"></canvas>
                </div>
                <div class="chart-card chart-card-full">
                    <h3>Amount Distribution (After Processing)</h3>
                    <canvas id="chart-amount-hist"></canvas>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        function switchPage(name, btn) {
            ...
            if (name === 'charts' && !window._chartsInit) { initCharts(); window._chartsInit = true; }
        }

        function switchTab(name, btn) {
            var parent = btn.closest('.section');
            parent.querySelectorAll('.tab-content').forEach(function(el) { el.classList.remove('active'); });
            parent.querySelectorAll('.tab').forEach(function(el) { el.classList.remove('active'); });
        ...
        function toggleTheme() {
            ...
            if (window._chartsInit) { destroyCharts(); initCharts(); }
        }
        ...
        (function() {
            var saved = localStorage.getItem('etl-dashboard-theme');
            if (saved) {
        ...
        function initCharts() {
            ...
            var mutedColor = getCSS('--text-muted');
            var borderColor = getCSS('--border');

            var catNames = {{ cat_names | tojson }};
            var catTotals = {{ cat_totals | tojson }};
            var catCounts = {{ cat_counts | tojson }};
            var statusLabels = {{ status_labels | tojson }};
            var statusCounts = {{ status_counts | tojson }};
            var transformLabels = {{ transform_labels | tojson }};
            var transformValues = {{ transform_values | tojson }};
            var amountBuckets = {{ amount_buckets | tojson }};
            var amountCounts = {{ amount_counts | tojson }};

            var palette = ['#3b82f6','#22c55e','#ef4444','#eab308','#f97316','#8b5cf6','#06b6d4','#ec4899'];
            Chart.defaults.color = mutedColor;

            chartInstances.push(new Chart(document.getElementById('chart-category-bar'), {
                type: 'bar',
                data: { labels: catNames, datasets: [{ label: 'Total ($)', data: catTotals, backgroundColor: palette.slice(0, catNames.length), borderRadius: 6 }] },
                options: { responsive: true, plugins: { legend: { display: false } }, scales: { x: { ticks: { color: mutedColor }, grid: { color: borderColor } }, y: { ticks: { color: mutedColor }, grid: { color: borderColor } } } }
            }));

            chartInstances.push(new Chart(document.getElementById('chart-status-pie'), {
                type: 'doughnut',
                data: { labels: statusLabels, datasets: [{ data: statusCounts, backgroundColor: ['#ef4444','#eab308','#22c55e'], borderWidth: 0 }] },
                options: { responsive: true, plugins: { legend: { position: 'bottom', labels: { color: textColor } } } }
            }));

            chartInstances.push(new Chart(document.getElementById('chart-category-count'), {
                type: 'bar',
                data: { labels: catNames, datasets: [{ label: 'Rows', data: catCounts, backgroundColor: 'rgba(59,130,246,0.7)', borderRadius: 6 }] },
                options: { responsive: true, plugins: { legend: { display: false } }, scales: { x: { ticks: { color: mutedColor }, grid: { color: borderColor } }, y: { ticks: { color: mutedColor, stepSize: 1 }, grid: { color: borderColor } } } }
            }));

            chartInstances.push(new Chart(document.getElementById('chart-transform'), {
                type: 'bar',
                data: { labels: transformLabels, datasets: [{ label: 'Count', data: transformValues, backgroundColor: ['#ef4444','#eab308','#22c55e','#3b82f6'], borderRadius: 6 }] },
                options: { indexAxis: 'y', responsive: true, plugins: { legend: { display: false } }, scales: { x: { ticks: { color: mutedColor, stepSize: 1 }, grid: { color: borderColor } }, y: { ticks: { color: mutedColor }, grid: { color: borderColor } } } }
            }));

            chartInstances.push(new Chart(document.getElementById('chart-amount-hist'), {
                type: 'bar',
                data: { labels: amountBuckets, datasets: [{ label: 'Transactions', data: amountCounts, backgroundColor: 'rgba(34,197,94,0.6)', borderColor: 'rgba(34,197,94,1)', borderWidth: 1, borderRadius: 4 }] },
                options: { responsive: true, plugins: { legend: { display: false } }, scales: { x: { ticks: { color: mutedColor }, grid: { color: borderColor } }, y: { ticks: { color: mutedColor, stepSize: 1 }, grid: { color: borderColor } } } }
            }));
        }
    </script>
...
def _build_chart_data(categories, diff_data, stats, processed_df):
    """Prepare data structures for Chart.js visualizations."""
    cat_names = [c["name"] for c in categories]
    cat_totals = [float(c["total"].replace(",", "")) for c in categories]
    cat_counts = [c["count"] for c in categories]

    status_map = {"Dropped": 0, "Modified": 0, "Unchanged": 0}
    for row in diff_data:
        s = row["status"]
        if s in status_map:
            status_map[s] += 1
    ...
    transform_values = [
        stats["rows_dropped"],
        stats["dates_fixed"],
        stats["categories_normalized"],
        stats["duplicates"],
    ]

    buckets = [0, 50, 100, 200, 500, 1000, float("inf")]
    bucket_labels = ["$0-50", "$50-100", "$100-200", "$200-500", "$500-1k", "$1k+"]
    bucket_counts = [0] * len(bucket_labels)
    if "amount" in processed_df.columns:
        amounts = processed_df["amount"].dropna()
        for amt in amounts:
    ...
    return {
        ...
        "amount_buckets": bucket_labels,
        "amount_counts": bucket_counts,
    }
...
def run_dashboard(config=None, host="127.0.0.1", port=5050):
    ...
    chart_data = _build_chart_data(categories, diff_data, stats, processed_df)
